...
const textData = computed(() => {
  if (isBackendData.value && backendData.value) {
    ...
    return {
      title: backendData.value.title || 'Текстовый блок',
      content: backendData.value.content || backendData.value.hidden_data || 'Данные загружены с сервера',
      editable: backendData.value.fate === 'editable'
    }
  }
  
  // Fallback на mock данные
  const mockParam = props.block.action_params?.find(p => p.variable === 'mock_content')
  if (mockParam?.data) {
    return {
      title: 'Текстовый блок',
      content: mockParam.data.content || 'Mock содержимое',
      editable: mockParam.data.editable || false
    }
  }
